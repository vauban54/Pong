...
public class PongUtils {
    ...
    private static double magnitude = sqrt(26); // racine carré de (INITIAL_VX au carré + INITIAL_VY)

    private static double deltaX = magnitude * cos(angle);
    private static double deltaY = magnitude * cos(angle);
    ...
    public static void handlePlayer(Rectangle player, double velY) {
        ...
        /*if ((player.setY(player.getY() >= 0) && (player.getY() <= 600)) {
            player.getY() + velY;
        }else {
            player.getY() + velY;
        }*/
        // (CONDITION ? TRUE : FALSE)
        //player.setY(player.getY() >= 0 && player.getY() <=600 ? + velY : player.getY());
    ...
    public static void updateGame(Rectangle player, Rectangle computer, Circle ball) {
        //ball.setCenterX(ball.getCenterX() + 3); -----> Pour essayer
        ...
        // Gestion de la collision de la ballesur la raquette du joueur
        if (player.getBoundsInParent().intersects(ball.getBoundsInParent())) {

            // On augmente la vitesse de la balle
            magnitude *= (magnitude < SPEED_LIMIT) ? ACCELERATION : 0.5;
            ...
            // On incrémente le score à chaque fois que la balle touche la raquette
